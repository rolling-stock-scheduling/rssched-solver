...
pub fn run(path: &str) {
    ...
    let mut local_search_solver = LocalSearch::initialize(config.clone(), units.clone(), nw.clone());
    ...
    println!();
    let loaded_schedule = Schedule::load_from_csv(&format!("{}{}", path, "leistungsketten.csv"), config.clone(), units.clone(), nw.clone());
